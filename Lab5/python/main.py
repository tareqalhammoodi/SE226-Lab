...
def rep_with_upper(String):
    randomCharacter = random.choice(String.lower())                         # select a random character to replace
    newString = String.replace(randomCharacter, randomCharacter.upper())    # replace that random char with uppercase
    return newString

def swap_letters(String):
    firstTwo = String[:2]                                                   # storing the first and second character
    lastTwo = String[-2:]                                                   # storing the last two characters
    swappedString = lastTwo + String[2:-2] + firstTwo
    return swappedString

def search_letter(password, letter):                                        # find the given letter in password
...
while True:
    userNumber = int(input("Please enter a number (between 3 and 7) : "))
    if userNumber < 3 or userNumber > 7:
        print("Invalid number, the number should be between 3-7.")
    else:
        print("\033[1mInitial password :\033[0m")
        Password = passGen(userNumber)
        print(Password)
        print("\033[1mFinal version of the password :\033[0m")
        print(swap_letters(rep_with_upper(Password)))
        userInput = input("Please enter the first letter of your name to check whether it's in the password or not : ")
        if search_letter(Password, userInput):                              # if ture
            print("The letter " + userInput + " has been found. (True)")
        else:                                                               # otherwise
            print("The letter " + userInput + " has not been found. (False)")
        break

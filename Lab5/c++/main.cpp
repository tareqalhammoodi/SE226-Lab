...
string words[1000] = {"a", "ability", "able", "about", "above", "accept", "according", "account", "across", "act",
                      ...
                      "all", "allow", "almost", "alone", "along", "already", "also", "although", "always",
                      "American",
                      ...
                      "boy", "break", "bring", "brother", "budget", "build", "building", "business", "but",
                      ...
                      "everything", "evidence", "exactly", "example",
                      ...
                      "information", "inside", "instead",
                      "institution", "interest", "interesting", "international", "interview", "into", "investment",
                      "involve", "issue", "it", "item", "its", "itself", "job", "join", "just", "keep", "key",
                      "kid", "kill",
                      ...
                      "my", "myself", "name", "nation", "national", "natural", "nature", "near", "nearly",
                      "necessary", "need", "network", "never", "new", "news", "newspaper", "next", "nice", "night",
                      "no",
                      "none", "nor", "north", "not", "note", "nothing", "notice", "now", "n't", "number", "occur",
                      "of", "off", "offer", "office", "officer", "official", "often", "oh", "oil",
                      "ok", "old", "on", "once", "one", "only", "onto", "open", "operation", "opportunity",
                      "option", "or", "order", "organization", "other", "others", "our", "out", "outside", "over",
                      "own", "owner", "page", "pain", "painting", "paper", "parent", "part", "participant",
                      ...
                      "prevent", "price", "private", "probably",
                      "problem", "process", "produce", "product", "production", "professional", "professor",
                      "program", "project", "property", "protect", "prove", "provide", "public", "pull", "purpose",
                      "push", "put", "quality", "question",
                      ...
                      "three", "through", "throughout", "throw", "thus", "time", "to", "today", "together",
                      "tonight",
                      "too", "top", "total", "tough", "toward", "town", "trade", "traditional", "training",
                      "travel", "treat", "treatment", "tree", "trial", "trip", "trouble", "true", "truth", "try",
                      "turn",
                      "TV", "two", "type", "under", "understand", "unit", "until", "up", "upon", "us", "use",
                      "usually", "value", "various", "very", "victim", "view", "violence", "visit", "voice",
                      "vote", "wait", "walk", "wall", "want", "war", "watch", "water", "way", "we", "weapon",
                      "wear", "week", "weight", "well", "west", "western", "what", "whatever", "when",
...
string passGen(int userInput) {
    string initialPassword = "";
    srand(time(NULL));
    for (int i = 0; i < userInput; i++) {
        string randomWords = words[rand() %1000];                                  // randomly choose words from the words list
        initialPassword += randomWords;                                            // merge these words into one string
    }
    string password(initialPassword.rbegin(), initialPassword.rend());   // reverse
    return password;
}

string rep_with_upper(string String) {
    srand(time(NULL));
    int randomIndex = rand() % String.length();                                     // generate random index
    char randomCharacter = String[randomIndex];                                     // get random character
    char upperCharacter = toupper(randomCharacter);                             // convert character to uppercase
    for (int i = 0; i < String.length(); i++) {                                    // replace it
        if (String[i] == randomCharacter) {
            String[i] = upperCharacter;
        }
    }
    return String;
}

string swap_letters(string String) {
    char temp = String[0];
    String[0] = String[String.length()-2];
    String[String.length()-2] = temp;
    temp = String[1];
...
bool search_letter(string password, string letter) {
    if (password.find(letter) != string::npos) {
        return true;
    } else {
        return false;
    }
...
int main() {
    ...
    while (true) {
        cout << "Please enter a number (between 3 and 7) : ";
        cin >> userNumber;
        if (userNumber < 3 or userNumber > 7) {
            cout << "Invalid number, the number should be between 3-7." << endl;
        } else {
            password = passGen(userNumber);
            cout << "Initial password : " << password << endl;
            cout << "Final password : " << swap_letters(rep_with_upper(password)) << endl;
            cout << "Please enter the first letter of your name to check whether it's in the password or not : " << endl;
            cin >> userInput;
            if (search_letter(password, userInput)) {                          // if true
                std::cout << "The letter " << userInput << " has been found. (True)" << std::endl;
            } else {                                                                // otherwise
                std::cout << "The letter " << userInput << " has not been found. (False)" << std::endl;
            }
            break;
        }
    }
